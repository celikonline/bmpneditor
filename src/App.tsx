...
function resolveAppPage(pathname: string): AppPage {
  ...
  if (pathname === '/runWorkflow') return 'run-workflow'
  ...
  const resourceRoutes: Record<string, PlatformView> = { '/agents': 'agents', '/humanTasks': 'human-tasks', '/workers': 'workers', '/userForms': 'user-forms', '/secrets': 'secrets', '/webhooks': 'webhooks', '/aiPrompts': 'ai-prompts', '/environment': 'environment', '/applications': 'applications', '/groups': 'groups', '/users': 'users', '/authentication': 'authentication' }
  if (resourceRoutes[pathname]) return resourceRoutes[pathname]
  return 'builder'
}

function platformPath(view: PlatformView) {
  return ({ executions: '/executions', 'execution-detail': '/execution', 'run-workflow': '/runWorkflow', queue: '/queueMonitor', events: '/eventMonitor', 'task-definitions': '/taskDefs', 'event-handlers': '/eventHandlers', schedulers: '/schedules', schemas: '/schemas', api: '/apiDocs', integrations: '/integrations', access: '/accessControl', agents: '/agents', 'human-tasks': '/humanTasks', workers: '/workers', 'user-forms': '/userForms', secrets: '/secrets', webhooks: '/webhooks', 'ai-prompts': '/aiPrompts', environment: '/environment', applications: '/applications', groups: '/groups', users: '/users', authentication: '/authentication' } as Record<PlatformView, string>)[view]
}
...
function App() {
  ...
  return (
    <div className="studio-shell">
      <StudioSidebar onOpenWorkflowList={openWorkflowList} onOpenBuilder={openBuilder} onNavigate={navigatePlatform} activePage={page} onAddBuilderTask={(taskName) => { const task = catalogItems.find((item) => item.name === taskName); if (task) addTask(task); else setDrawerOpen(true) }} onOpenBuilderTab={(tab) => { setDrawerOpen(false); setActiveTab(tab) }} onRunBuilder={() => setExecuteOpen(true)} onSaveBuilder={saveWorkflow} onPublishBuilder={publishVersion} onSearchBuilder={() => setCanvasSearchOpen(true)} />
...
function StudioSidebar({ onOpenWorkflowList, onOpenBuilder, onNavigate, activePage, onAddBuilderTask, onOpenBuilderTab, onRunBuilder, onSaveBuilder, onPublishBuilder, onSearchBuilder }: { onOpenWorkflowList: () => void; onOpenBuilder: (entry?: BuilderEntry) => void; onNavigate: (view: PlatformView) => void; activePage: AppPage; onAddBuilderTask?: (taskName: string) => void; onOpenBuilderTab?: (tab: 'Code' | 'Run' | 'Dependencies') => void; onRunBuilder?: () => void; onSaveBuilder?: () => void; onPublishBuilder?: () => void; onSearchBuilder?: () => void }) {
  ...
  const commands: Array<{ name: string; detail: string; run: () => void }> = [
    ...(onAddBuilderTask ? [{ name: 'Add HTTP Task', detail: 'Insert an HTTP task into the workflow', run: () => onAddBuilderTask('HTTP Task') }, { name: 'Add Switch', detail: 'Insert a decision switch into the workflow', run: () => onAddBuilderTask('Switch') }] : []),
    ...
    ...(onRunBuilder ? [{ name: 'Run workflow', detail: 'Start the current draft with input', run: onRunBuilder }] : []),
    ...(onSaveBuilder ? [{ name: 'Save workflow', detail: 'Save the current draft', run: onSaveBuilder }] : []),
    ...(onPublishBuilder ? [{ name: 'Publish workflow', detail: 'Publish the current validated version', run: onPublishBuilder }] : []),
    ...(onOpenBuilderTab ? [{ name: 'Open Code', detail: 'Inspect and edit workflow JSON', run: () => onOpenBuilderTab('Code') }, { name: 'Open Dependencies', detail: 'Review integrations and resources', run: () => onOpenBuilderTab('Dependencies') }] : []),
    ...(onSearchBuilder ? [{ name: 'Search nodes', detail: 'Find a task or route on the canvas', run: onSearchBuilder }] : []),
    ...
    ...(!onRunBuilder ? [{ name: 'Run workflow', detail: 'Start a workflow with input', run: () => onNavigate('run-workflow') }] : []),
...
function WorkflowList({ onOpen, onOpenWorkflow, onImportJson }: { onOpen: (entry?: BuilderEntry) => void; onOpenWorkflow: (name: string) => void; onImportJson: (file?: File) => void }) {
  ...
  const [statusFilter, setStatusFilter] = useState<'ALL' | WorkflowDefinitionRecord['status']>('ALL')
  ...
  const visible = workflows.filter((workflow) => `${workflow.name} ${workflow.description} ${workflow.status}`.toLowerCase().includes(query.toLowerCase()) && (statusFilter === 'ALL' || workflow.status === statusFilter))
