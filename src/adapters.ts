...
function configFromConductorTask(task: ConductorCompatibleTask): TaskConfig {
  ...
  if (task.type.startsWith('LLM_')) { ;(['llmProvider', 'model', 'promptName', 'instructions', 'text', 'vectorDB', 'index', 'namespace', 'embeddingModelProvider', 'embeddingModel', 'docId', 'query', 'mediaType'] as const).forEach((key) => assign(key, text(input[key]))); ;(['messages', 'promptVariables', 'stopWords', 'jsonOutput', 'embeddings'] as const).forEach((key) => assign(key, jsonText(input[key]))); ;(['temperature', 'topP', 'maxTokens', 'dimensions', 'maxResults', 'chunkSize', 'chunkOverlap'] as const).forEach((key) => assign(key, input[key] == null ? undefined : Number(input[key]))) }
