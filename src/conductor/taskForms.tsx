...
function MediaFields({ kind, config, updateConfig }: Pick<TaskFormProps, 'kind' | 'config' | 'updateConfig'>) {
  const image = kind === 'GENERATE_IMAGE' || kind === 'AI_GENERATE_IMAGE'
  const audio = kind === 'GENERATE_AUDIO'
  const video = kind === 'GENERATE_VIDEO'
  return <><div className="compact-fields"><div><Input label="LLM provider" value={config.llmProvider ?? ''} placeholder="openai" onChange={(value) => updateConfig({ llmProvider: value })} /></div><div><Input label="Model" value={config.model ?? ''} placeholder="model-name" onChange={(value) => updateConfig({ model: value })} /></div></div>{audio ? <Textarea label="Text" value={config.text ?? ''} placeholder="Text to convert to speech" onChange={(value) => updateConfig({ text: value })} /> : kind === 'GENERATE_PDF' ? <Textarea label="Markdown" value={config.markdown ?? ''} placeholder="# Markdown content to render as a PDF" onChange={(value) => updateConfig({ markdown: value })} /> : <Textarea label="Prompt" value={config.prompt ?? ''} placeholder={image ? 'Describe the image to generate' : 'Describe the video to generate'} onChange={(value) => updateConfig({ prompt: value })} />}{video && <Input label="Input image (optional)" value={config.inputImage ?? ''} placeholder="${workflow.input.imageUrl}" onChange={(value) => updateConfig({ inputImage: value })} />}{image && <div className="compact-fields"><div><Input label="Number of images (n)" type="number" value={config.n ?? 1} onChange={(value) => updateConfig({ n: value ? Number(value) : undefined })} /></div><div><Input label="Width" type="number" value={config.width ?? 1024} onChange={(value) => updateConfig({ width: value ? Number(value) : undefined })} /></div><div><Input label="Height" type="number" value={config.height ?? 1024} onChange={(value) => updateConfig({ height: value ? Number(value) : undefined })} /></div></div>}{audio && <div className="compact-fields"><div><Input label="Voice" value={config.voice ?? ''} placeholder="alloy" onChange={(value) => updateConfig({ voice: value })} /></div><div><Input label="Speed" type="number" value={config.speed ?? 1} onChange={(value) => updateConfig({ speed: value ? Number(value) : undefined })} /></div><div><Input label="Response format" value={config.responseFormat ?? ''} placeholder="mp3" onChange={(value) => updateConfig({ responseFormat: value })} /></div></div>}{video && <><div className="compact-fields"><div><Input label="Aspect ratio" value={config.aspectRatio ?? ''} placeholder="16:9" onChange={(value) => updateConfig({ aspectRatio: value })} /></div><div><Input label="Resolution" value={config.resolution ?? ''} placeholder="1080p" onChange={(value) => updateConfig({ resolution: value })} /></div><div><Input label="FPS" type="number" value={config.fps ?? 24} onChange={(value) => updateConfig({ fps: value ? Number(value) : undefined })} /></div></div><div className="compact-fields"><div><Input label="Duration (sec)" type="number" value={config.duration ?? 10} onChange={(value) => updateConfig({ duration: value ? Number(value) : undefined })} /></div><div><Input label="Output format" value={config.outputFormat ?? 'mp4'} onChange={(value) => updateConfig({ outputFormat: value })} /></div><div><Input label="Seed" type="number" value={config.seed ?? ''} onChange={(value) => updateConfig({ seed: value ? Number(value) : undefined })} /></div></div><div className="compact-fields"><label className="form-toggle"><input type="checkbox" checked={config.generateAudio ?? false} onChange={(event) => updateConfig({ generateAudio: event.target.checked })} /><span>Generate audio</span></label><label className="form-toggle"><input type="checkbox" checked={config.generateThumbnail ?? false} onChange={(event) => updateConfig({ generateThumbnail: event.target.checked })} /><span>Generate thumbnail</span></label></div></>}{kind === 'GENERATE_PDF' && <><div className="compact-fields"><div><Input label="Page size" value={config.pageSize ?? 'A4'} onChange={(value) => updateConfig({ pageSize: value })} /></div><div><Input label="Theme" value={config.theme ?? 'default'} onChange={(value) => updateConfig({ theme: value })} /></div><div><Input label="Base font size" type="number" value={config.baseFontSize ?? 12} onChange={(value) => updateConfig({ baseFontSize: value ? Number(value) : undefined })} /></div></div><div className="compact-fields"><div><Input label="Margin top" type="number" value={config.marginTop ?? 36} onChange={(value) => updateConfig({ marginTop: value ? Number(value) : undefined })} /></div><div><Input label="Margin right" type="number" value={config.marginRight ?? 36} onChange={(value) => updateConfig({ marginRight: value ? Number(value) : undefined })} /></div><div><Input label="Margin bottom" type="number" value={config.marginBottom ?? 36} onChange={(value) => updateConfig({ marginBottom: value ? Number(value) : undefined })} /></div><div><Input label="Margin left" type="number" value={config.marginLeft ?? 36} onChange={(value) => updateConfig({ marginLeft: value ? Number(value) : undefined })} /></div></div><Textarea label="Document metadata (JSON)" value={config.pdfMetadata ?? ''} placeholder={'{"title":"Report"}'} onChange={(value) => updateConfig({ pdfMetadata: value })} /><div className="compact-fields"><div><Input label="Output location" value={config.outputLocation ?? ''} placeholder="s3://bucket/report.pdf" onChange={(value) => updateConfig({ outputLocation: value })} /></div><div><Input label="Image base URL" value={config.imageBaseUrl ?? ''} onChange={(value) => updateConfig({ imageBaseUrl: value })} /></div></div></>}</>
}

function McpFields({ kind, config, updateConfig }: Pick<TaskFormProps, 'kind' | 'config' | 'updateConfig'>) {
  return <><Input label="MCP server URL" value={config.mcpServer ?? ''} placeholder="https://mcp.example.com" onChange={(value) => updateConfig({ mcpServer: value })} />{kind === 'CALL_MCP_TOOL' && <><Input label="Tool name (method)" value={config.operation ?? ''} placeholder="search_documents" onChange={(value) => updateConfig({ operation: value })} /><Textarea label="Arguments (JSON or variable reference)" value={config.arguments ?? ''} placeholder={'${workflow.input.toolArgs}'} onChange={(value) => updateConfig({ arguments: value })} /></>}<Textarea label="Headers (JSON object)" value={config.headers ?? ''} placeholder={'{"authorization":"Bearer ${workflow.input.token}"}'} onChange={(value) => updateConfig({ headers: value })} /></>
}
...
export function TaskFormFields({ kind, config, optional, updateConfig, updateData }: TaskFormProps): ReactNode {
  ...
  if (kind === 'LIST_MCP_TOOLS' || kind === 'CALL_MCP_TOOL') return <McpFields kind={kind} config={config} updateConfig={updateConfig} />
  ...
  if (kind === 'AI_CHAT_COMPLETION' || kind === 'IA_TASK') return <AiFields config={config} updateConfig={updateConfig} />
  if (['AI_GENERATE_IMAGE', 'GENERATE_IMAGE', 'GENERATE_AUDIO', 'GENERATE_VIDEO', 'GENERATE_PDF'].includes(kind)) return <MediaFields kind={kind} config={config} updateConfig={updateConfig} />
  if (kind === 'UPDATE_SECRET') return <UpdateSecretFields config={config} updateConfig={updateConfig} />
  if (kind === 'TASK_SUMMARY' || kind === 'LAMBDA' || kind === 'USER_DEFINED' || kind === 'JUMP') return <GenericFields kind={kind} config={config} updateConfig={updateConfig} />
