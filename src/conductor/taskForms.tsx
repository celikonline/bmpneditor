...
export function TaskFormFields({ kind, config, optional, updateConfig, updateData }: TaskFormProps): ReactNode {
  ...
  if (kind === 'EVENT' || kind === 'KAFKA_PUBLISH' || kind === 'WAIT_FOR_EVENT') return <><Input label="Event name / topic" value={config.eventName ?? ''} placeholder="workflow.events.job.completed" onChange={(value) => updateConfig({ eventName: value })} /><Textarea label="Event payload" value={config.eventPayload ?? ''} placeholder="JSON event payload" onChange={(value) => updateConfig({ eventPayload: value })} /></>
  if (kind === 'GRPC') return <Input label="Service method" value={config.serviceMethod ?? ''} placeholder="package.Service/Method" onChange={(value) => updateConfig({ serviceMethod: value })} />
  ...
  if (kind === 'BUSINESS_RULE') return <Input label="Rule name" value={config.ruleName ?? ''} placeholder="order_eligibility" onChange={(value) => updateConfig({ ruleName: value })} />
  if (kind === 'SQL' || kind === 'JDBC') return <Textarea label="SQL query" value={config.sqlQuery ?? ''} placeholder="SELECT * FROM jobs WHERE id = :jobId" onChange={(value) => updateConfig({ sqlQuery: value })} />
  ...
  if (kind === 'AI_CHAT_COMPLETION' || kind === 'AI_GENERATE_IMAGE' || kind === 'IA_TASK' || kind.startsWith('LLM_') || ['AGENT', 'GET_AGENT_CARD', 'CANCEL_AGENT', 'LIST_MCP_TOOLS', 'CALL_MCP_TOOL', 'GENERATE_IMAGE', 'GENERATE_AUDIO', 'GENERATE_VIDEO', 'GENERATE_PDF'].includes(kind)) return <AiFields config={config} updateConfig={updateConfig} />
  if (kind === 'UPDATE_SECRET' || kind === 'LIST_MCP_TOOLS' || kind === 'CALL_MCP_TOOL' || kind === 'TASK_SUMMARY' || kind === 'LAMBDA' || kind === 'USER_DEFINED' || kind === 'JUMP') return <GenericFields kind={kind} config={config} updateConfig={updateConfig} />
