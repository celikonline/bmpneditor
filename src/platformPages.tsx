...
function TaskDefinitionsPage() {
  ...
  const openNew = () => { setEditingName(null); setDraft(emptyDraft); setFormOpen(true) }
  const edit = (item: TaskDefinitionRecord) => { setEditingName(item.name); setDraft({ name: item.name, description: item.description, owner: item.owner, timeoutSeconds: String(item.timeoutSeconds), retryCount: String(item.retryCount), retryLogic: item.retryLogic ?? 'FIXED', retryDelaySeconds: String(item.retryDelaySeconds ?? 0), maxRetryDelaySeconds: String(item.maxRetryDelaySeconds ?? 0), backoffJitterMs: String(item.backoffJitterMs ?? 0), totalTimeoutSeconds: String(item.totalTimeoutSeconds ?? 0), responseTimeoutSeconds: String(item.responseTimeoutSeconds ?? 0), pollTimeoutSeconds: String(item.pollTimeoutSeconds ?? 0) }); setFormOpen(true) }
  const save = () => { if (!draft.name.trim()) return; void workflowApi.saveTaskDefinition({ name: draft.name.trim(), description: draft.description, owner: draft.owner, timeoutSeconds: Number(draft.timeoutSeconds) || 60, retryCount: Number(draft.retryCount) || 0, retryLogic: draft.retryLogic, retryDelaySeconds: Number(draft.retryDelaySeconds) || 0, maxRetryDelaySeconds: Number(draft.maxRetryDelaySeconds) || 0, backoffJitterMs: Number(draft.backoffJitterMs) || 0, totalTimeoutSeconds: Number(draft.totalTimeoutSeconds) || 0, responseTimeoutSeconds: Number(draft.responseTimeoutSeconds) || 0, pollTimeoutSeconds: Number(draft.pollTimeoutSeconds) || 0, updatedAt: new Date().toISOString(), status: 'ACTIVE' }).then(() => { setFormOpen(false); setEditingName(null); setDraft(emptyDraft); refresh() }) }
...
function EventHandlersPage() {
  ...
  useEffect(() => { refresh() }, [])
  const openNew = () => { setEditingName(null); setDraft({ name: '', event: '', action: 'START_WORKFLOW', workflowName: 'api_polling_workflow' }); setFormOpen(true) }
  const edit = (item: EventHandlerRecord) => { setEditingName(item.name); setDraft({ name: item.name, event: item.event, action: item.action, workflowName: item.workflowName }); setFormOpen(true) }
  const save = () => { if (!draft.name.trim() || !draft.event.trim()) return; void workflowApi.saveEventHandler({ ...draft, name: draft.name.trim(), event: draft.event.trim(), active: items.find((item) => item.name === editingName)?.active ?? true, updatedAt: new Date().toISOString() }).then(() => { setFormOpen(false); setEditingName(null); refresh() }) }
