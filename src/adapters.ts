...
function configFromConductorTask(task: ConductorCompatibleTask): TaskConfig {
  ...
  if (task.type === 'HTTP') { assign('url', text(input.uri)); assign('method', text(input.method)); assign('accept', text(input.accept)); assign('contentType', text(input.contentType)); assign('headers', jsonText(input.headers)); assign('body', jsonText(input.body)); assign('encode', input.encode == null ? undefined : Boolean(input.encode)); assign('httpHedgingMaxAttempts', Number((input.hedgingConfig as Record<string, unknown> | undefined)?.maxAttempts ?? 0) || undefined) }
  if (task.type === 'HTTP_POLL' && request) { assign('url', text(request.uri)); assign('method', text(request.method)); assign('accept', text(request.accept)); assign('contentType', text(request.contentType)); assign('headers', jsonText(request.headers)); assign('body', jsonText(request.body)); assign('encode', request.encode == null ? undefined : Boolean(request.encode)); assign('httpHedgingMaxAttempts', Number((request.hedgingConfig as Record<string, unknown> | undefined)?.maxAttempts ?? 0) || undefined); assign('pollCondition', text(request.terminationCondition)); assign('pollIntervalSeconds', Number(request.pollingInterval ?? 30)) }
  if (task.type === 'EVENT' || task.type === 'WAIT_FOR_EVENT') { assign('eventName', text(input.sink)); assign('eventPayload', jsonText(input.eventPayload)) }
  if (task.type === 'KAFKA_PUBLISH') { const kafka = input.kafka_request as Record<string, unknown> | undefined; assign('kafkaTopic', text(kafka?.topic)); assign('eventName', text(kafka?.topic)); assign('kafkaValue', text(kafka?.value)); assign('eventPayload', text(kafka?.value)); assign('kafkaBootStrapServers', text(kafka?.bootStrapServers)); assign('kafkaHeaders', jsonText(kafka?.headers)); assign('kafkaKey', text(kafka?.key)); assign('kafkaKeySerializer', text(kafka?.keySerializer)) }
  if (task.type === 'GRPC') { assign('grpcService', text(input.service)); assign('grpcMethod', text(input.method)); assign('serviceMethod', text(input.method ?? input.serviceMethod)); assign('grpcHost', text(input.host)); assign('grpcPort', input.port == null ? undefined : Number(input.port)); assign('grpcUseSSL', input.useSSL == null ? undefined : Boolean(input.useSSL)); assign('grpcTrustCert', input.trustCert == null ? undefined : Boolean(input.trustCert)); assign('grpcRequest', jsonText(input.request)); assign('grpcHeaders', jsonText(input.headers)); assign('grpcInputType', text(input.inputType)); assign('grpcMethodType', text(input.methodType)); assign('grpcOutputType', text(input.outputType)); assign('grpcHedgingMaxAttempts', Number((input.hedgingConfig as Record<string, unknown> | undefined)?.maxAttempts ?? 0) || undefined) }
  ...
  if (task.type === 'BUSINESS_RULE') { assign('ruleFileLocation', text(input.ruleFileLocation)); assign('executionStrategy', text(input.executionStrategy)); assign('inputColumns', jsonText(input.inputColumns)); assign('outputColumns', jsonText(input.outputColumns)); assign('cacheTimeoutMinutes', input.cacheTimeoutMinutes == null ? undefined : Number(input.cacheTimeoutMinutes)) }
  if (task.type === 'SQL' || task.type === 'JDBC') { assign('integrationName', text(input.integrationName)); assign('schemaName', text(input.schemaName)); assign('sqlQuery', text(input.statement)); assign('jdbcParameters', jsonText(input.parameters)); assign('databaseType', text(input.type) as TaskConfig['databaseType']); assign('expectedUpdateCount', text(input.expectedUpdateCount)) }
  ...
  if (task.type === 'UPDATE_SECRET') { const secrets = input._secrets as Record<string, unknown> | undefined; assign('secretKey', text(secrets?.secretKey)); assign('secretValue', text(secrets?.secretValue)) }
  ...
  if (task.type === 'AGENT' || task.type === 'GET_AGENT_CARD' || task.type === 'CANCEL_AGENT') { ;(['agentType', 'agentName', 'agentVersion', 'agentUrl', 'taskId', 'text', 'modelOverride', 'contextId'] as const).forEach((key) => assign(key, text(input[key]))); ;(['streaming', 'pushNotification'] as const).forEach((key) => assign(key, input[key] == null ? undefined : Boolean(input[key]))); ;(['pollIntervalSeconds', 'maxDurationSeconds', 'maxPollFailures', 'historyLength'] as const).forEach((key) => assign(key, input[key] == null ? undefined : Number(input[key]))) }
  ...
  if (task.type === 'LIST_MCP_TOOLS' || task.type === 'CALL_MCP_TOOL') { assign('mcpServer', text(input.mcpServer)); assign('operation', text(input.method)); assign('headers', jsonText(input.headers)); assign('arguments', jsonText(input.arguments)) }
  if (task.type === 'GENERATE_IMAGE' || task.type === 'AI_GENERATE_IMAGE' || task.type === 'GENERATE_AUDIO' || task.type === 'GENERATE_VIDEO' || task.type === 'GENERATE_PDF') {
    ;(['llmProvider', 'model', 'prompt', 'text', 'voice', 'responseFormat', 'inputImage', 'aspectRatio', 'resolution', 'size', 'style', 'outputFormat', 'personGeneration', 'motion', 'negativePrompt', 'markdown', 'pageSize', 'theme', 'outputLocation', 'imageBaseUrl'] as const).forEach((key) => assign(key, text(input[key]))); assign('pdfMetadata', jsonText(input.pdfMetadata)); ;(['n', 'width', 'height', 'weight', 'speed', 'fps', 'duration', 'seed', 'thumbnailTimestamp', 'guidanceScale', 'maxDurationSeconds', 'maxCostDollars', 'baseFontSize', 'marginTop', 'marginRight', 'marginBottom', 'marginLeft'] as const).forEach((key) => assign(key, input[key] == null ? undefined : Number(input[key]))); ;(['generateAudio', 'generateThumbnail'] as const).forEach((key) => assign(key, input[key] == null ? undefined : Boolean(input[key])))
  }
  if (task.type.startsWith('LLM_')) { ;(['llmProvider', 'model', 'promptName', 'instructions', 'text', 'vectorDB', 'index', 'namespace', 'embeddingModelProvider', 'embeddingModel', 'docId', 'query', 'mediaType'] as const).forEach((key) => assign(key, text(input[key]))); ;(['messages', 'promptVariables', 'stopWords', 'jsonOutput', 'embeddings', 'metadata'] as const).forEach((key) => assign(key, jsonText(input[key]))); ;(['temperature', 'topP', 'maxTokens', 'dimensions', 'maxResults', 'chunkSize', 'chunkOverlap'] as const).forEach((key) => assign(key, input[key] == null ? undefined : Number(input[key]))) }
