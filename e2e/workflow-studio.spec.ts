...
test('creates and removes task definitions and schedules', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Edit task definition email_worker', exact: true }).click()
  await expect(page.getByRole('heading', { name: 'Edit task definition', exact: true })).toBeVisible()
  await page.getByLabel('Description').fill('Sends an updated transactional email')
  await page.getByRole('button', { name: 'Save definition', exact: true }).click()
  await expect(page.getByText('Sends an updated transactional email', { exact: true })).toBeVisible()
  page.once('dialog', (dialog) => dialog.accept())
...
test('runs a workflow from the platform Run Workflow screen', async ({ page }) => {
  ...
  await expect(page.locator('.task-execution-summary')).toContainText('Task ID:')
